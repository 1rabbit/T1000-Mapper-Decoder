// Sensecap T1000 decoder for mapping and more - 2024 1rabbit
function decodeUplink(input) {

    let decoded = {}

    const decodedFull = original_decodeUplink(input).data

    if (typeof decodedFull.messages !== 'undefined') {

        let timestampMillis = 0

        decodedFull.messages[0].forEach(message => {

            if (message.type === "Event Status") {
                // Extract event name
                if (typeof message.measurementValue[0]?.eventName !== 'undefined') decoded.event = message.measurementValue[0].eventName.replace('.', '')
            }
            else {
                // Extract measurement values
                if (typeof message.measurementValue !== 'undefined') {
                    decoded[message.type.toLowerCase().replace(/\s+/g, '_').replace('air_', '')] = parseFloat(message.measurementValue)
                }
        ...
        if (Date.now() - timestampMillis < 30 * 1000 && typeof decoded.latitude === 'number') { // data is less than 30s old
            decoded.freshGPS = true
            decoded.accuracy = 2      // adding mandatory values for mapping when gps is fresh
            decoded.altitude = 0
        }
        else decoded.freshGPS = false
...
function original_decodeUplink(input) { // Original function untouched, renamed to original_decodeUplink
// function decodeUplink (input) {
